...
Title.TextSize = 18
Title.Text = 'inf-deep ' .. currentVersion
...
EventBind =
	makeSettingsButton('Edit Event Binds', 'rbxassetid://5147695474', 759)
...
ViewportTextBox = (function()
	...
	local function convert(textbox)
		...
		textbox.Changed:Connect(function(prop)
			if
				prop == 'Text'
				or prop == 'CursorPosition'
				or prop == 'AbsoluteSize'
			then
				obj:Update()
			end
		end)
...
function r15(plr)
	if
		plr.Character:FindFirstChildOfClass('Humanoid').RigType
...
function dragGUI(gui)
	task.spawn(function()
		...
		local function update(input)
			...
			TweenService
				:Create(gui, TweenInfo.new(0.20), { Position = Position })
				:Play()
		end
			...
			for i, cmd in pairs(event.commands) do
				...
				for idx, set in pairs(event.sets) do
					...
					if condType == 'Player' then
						if cmdSet == 0 then
							metCondition = metCondition
								and (tostring(Players.LocalPlayer) == argVal)
						elseif cmdSet ~= 1 then
							metCondition = metCondition
								and table.find(
									getPlayer(cmdSet, Players.LocalPlayer),
									argVal
								)
						end
					elseif condType == 'String' then
						if cmdSet ~= 0 then
							metCondition = metCondition
								and string.find(argVal:lower(), cmdSet:lower())
						end
					elseif condType == 'Number' then
						if cmdSet ~= 0 then
							metCondition = metCondition
								and tonumber(argVal) <= tonumber(cmdSet)
						end
					end
		...
		for i, v in pairs(eventListHolder:GetChildren()) do
			if v.Name == 'EventTemplate' then
				...
				if v.Expand.Rotation == 90 then
					size = size
						+ 20
							* (1 + (#events[v.EventName:GetAttribute('RawName')].commands or 0))
				end
			end
		end
	...
	local function refreshList()
		...
		for name, event in pairs(events) do
				...
				eventF:TweenSize(
					UDim2.new(
						...
						20
							+ (
								expanded
									and 20 * #eventF.Cmds.Holder:GetChildren()
								or 0
							)
					),
					Enum.EasingDirection.Out,
			...
			local function refreshCommands()
				...
				eventF.EventName.Text = name
					.. (
						#event.commands > 0 and ' (' .. #event.commands .. ')'
						or ''
					)
			...
			if readfile('IY_FE.iy') ~= nil then
				local success, response = pcall(function()
					...
					if json.currentScroll ~= nil then
						currentScroll = Color3.new(
							json.currentScroll[1],
...
function maximizeHolder()
	if StayOpen == false then
		Holder:TweenPosition(
			UDim2.new(1, Holder.Position.X.Offset, 1, -220),
			'InOut',
			'Quart',
			0.2,
			true,
...
function minimizeHolder()
	if StayOpen == false then
		Holder:TweenPosition(
			UDim2.new(1, Holder.Position.X.Offset, 1, minimizeNum),
			'InOut',
			'Quart',
			0.5,
			true,
...
function cmdbarHolder()
	if StayOpen == false then
		Holder:TweenPosition(
			UDim2.new(1, Holder.Position.X.Offset, 1, -45),
			'InOut',
			'Quart',
			0.5,
			true,
...
function CreateJoinLabel(plr, ID)
	...
	ImageLabel_3.Image = Players:GetUserThumbnailAsync(
		ID,
		...
		do
			...
			ColorPicker.new = function()
				...
				for i, v in pairs(basicColors) do
					...
					newColor.Position =
						UDim2.new(0, 1 + 30 * column, 0, 21 + 23 * row)
				...
				for i = 1, 12 do
					...
					newColor.Position =
						UDim2.new(0, 1 + 30 * column, 0, 20 + 23 * row)
	...
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local function nameFile()
				...
				pcall(function()
					file = readfile(
						placeName .. ' Chat Logs (' .. fileext .. ').txt'
					)
				end)
				if file then
					fileext = fileext + 1
...
Players.PlayerRemoving:Connect(function(player)
	if
		ESPenabled
		or CHMSenabled
		or COREGUI:FindFirstChild(player.Name .. '_LC')
	then
...
EventBind.MouseButton1Click:Connect(function()
	eventEditor.Frame:TweenPosition(
		UDim2.new(0.5, -175, 0.5, -101),
		'InOut',
		'Quart',
		0.5,
		true,
...
Add.MouseButton1Click:Connect(function()
	KeybindEditor:TweenPosition(
		UDim2.new(0.5, -180, 0, 260),
		'InOut',
		'Quart',
		0.5,
		true,
...
function selectPart()
	ToPartFrame:TweenPosition(
		UDim2.new(0.5, -180, 0, 335),
		'InOut',
		'Quart',
		0.5,
		true,
	...
	if Path.Text ~= '' then
		...
		local function handleWpNames()
			...
			for i, v in pairs(pWayPoints) do
				if
					v.NAME:lower()
					== selected.Adornee.Name:lower() .. tpNameExt
				then
					FoundDupe = true
				end
			end
			if not FoundDupe then
				notify(
					'Modified Waypoints',
					'Created waypoint: ' .. selected.Adornee.Name .. tpNameExt
				)
...
function dragMain(dragpoint, gui)
	task.spawn(function()
		...
		local function update(input)
			...
			if
				startPos.X.Offset + delta.X <= -250
				and -CamViewport() <= startPos.X.Offset + delta.X
			then
				local Position = UDim2.new(
					startPos.X.Scale,
					startPos.X.Offset + delta.X,
...
task.spawn(function()
	local success, result = pcall(function()
		chatbox =
			game.WaitForChild(PlayerGui, 'Chat').Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar
	end)
	if success then
		...
		local function Index()
			if chatbox.Text:lower():sub(1, 1) == prefix then
				...
				IndexContents(
					PlayerGui.Chat.Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar.Text
						:lower()
						:sub(2),
					true
				)
		...
		PlayerGui:WaitForChild('Chat').Frame.ChatBarParentFrame.ChildAdded
			:Connect(function(newbar)
				wait()
				if newbar:FindFirstChild('BoxFrame') then
					chatbox =
						PlayerGui:WaitForChild('Chat').Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar
...
CMDs[#CMDs + 1] =
	{ NAME = 'lookat [plr]', DESC = 'Moves your camera view to a player' }
CMDs[#CMDs + 1] = { NAME = '', DESC = '' }
CMDs[#CMDs + 1] = {
	NAME = 'btools (CLIENT)',
...
CMDs[#CMDs + 1] =
	{ NAME = 'userid / id [plr]', DESC = 'Notifies a players user ID' }
CMDs[#CMDs + 1] = {
...
CMDs[#CMDs + 1] =
	{ NAME = 'unfriend [plr]', DESC = 'Unfriends certain players' }
...
CMDs[#CMDs + 1] =
	{ NAME = 'stareat / stare [plr]', DESC = 'Stare / look at a player' }
CMDs[#CMDs + 1] =
	{ NAME = 'unstareat / unstare [plr]', DESC = 'Disables stareat' }
CMDs[#CMDs + 1] = {
...
CMDs[#CMDs + 1] =
	{ NAME = 'kill [plr] (TOOL)', DESC = 'Kills a player (YOU NEED A TOOL)' }
CMDs[#CMDs + 1] = {
...
CMDs[#CMDs + 1] =
	{ NAME = 'muteboombox [plr]', DESC = 'Mutes someones boombox' }
CMDs[#CMDs + 1] =
	{ NAME = 'unmuteboombox [plr]', DESC = 'Unmutes someones boombox' }
CMDs[#CMDs + 1] = {
...
CMDs[#CMDs + 1] =
	{ NAME = 'copytools [plr] (CLIENT)', DESC = 'Copies a players tools' }
CMDs[#CMDs + 1] = {
...
CMDs[#CMDs + 1] = {
	NAME = 'nolighting / nolight',
	DESC = 'Disables all lighting in the game',
}
wait()
...
function refresh(plr)
	...
	local Human = plr.Character
		and plr.Character:FindFirstChildOfClass('Humanoid', true)
	...
	task.spawn(function()
		...
		then
			Players.LocalPlayer.Character
				:FindFirstChildOfClass('Humanoid').Died
				:Connect(function()
...
findCmd = function(cmd_name)
	for i, v in pairs(cmds) do
		if
			v.NAME:lower() == cmd_name:lower()
			or FindInTable(v.ALIAS, cmd_name:lower())
		then
			return v
		end
	end
		...
		then
			for k, x in pairs(v.Character:GetChildren()) do
				if string.find(x.Name, 'Torso') then
					local Distance = (
						WorldToScreen(x) - MousePositionToVector2()
					).Magnitude
...
SpecialPlayerCases = {
	...
	['%%(.+)'] = function(speaker, args)
		...
		for _, plr in pairs(Players:GetPlayers()) do
			if
				plr.Team
				and string.sub(string.lower(plr.Team.Name), 1, #team)
					== string.lower(team)
			then
	...
	['bacons'] = function(speaker, args)
		...
		for _, plr in pairs(Players:GetPlayers()) do
			if
				plr.Character:FindFirstChild('Pal Hair')
				or plr.Character:FindFirstChild('Kate Hair')
			then
				table.insert(returns, plr)
			end
		end
	...
	['alive'] = function(speaker, args)
		...
		for _, plr in pairs(Players:GetPlayers()) do
			if
				...
				and plr.Character:FindFirstChildOfClass('Humanoid').Health
					> 0
			then
				table.insert(returns, plr)
			end
	...
	['dead'] = function(speaker, args)
		...
		for _, plr in pairs(Players:GetPlayers()) do
			if
				...
				or plr.Character:FindFirstChildOfClass('Humanoid').Health
					<= 0
			then
				table.insert(returns, plr)
			end
	...
	['rad(%d+)'] = function(speaker, args)
		...
		for _, plr in pairs(Players:GetPlayers()) do
			if plr.Character and getRoot(plr.Character) then
				local magnitude = (getRoot(plr.Character).Position - getRoot(
					speakerChar
				).Position).magnitude
		...
		for i, v in pairs(tokens) do
			if v.Operator == '+' then
				...
				for regex, case in pairs(SpecialPlayerCases) do
					...
					if #matches > 0 then
						foundCase = true
						initialPlayers = onlyIncludeInTable(
							initialPlayers,
							case(speaker, matches, initialPlayers)
						)
					end
				end
				...
				for regex, case in pairs(SpecialPlayerCases) do
					...
					if #matches > 0 then
						foundCase = true
						initialPlayers = removeTableMatches(
							initialPlayers,
							case(speaker, matches, initialPlayers)
						)
					end
...
UserInputService.TextBoxFocused:Connect(function(obj)
	...
	lastTextBoxCon = obj:GetPropertyChangedSignal('Text'):Connect(function()
		if
			not (
				UserInputService:IsKeyDown(Enum.KeyCode.Return)
				or UserInputService:IsKeyDown(Enum.KeyCode.KeypadEnter)
			)
		then
			lastTextBoxString = obj.Text
		end
...
Cmdbar.Focused:Connect(function()
	...
	tabComplete = UserInputService.InputBegan:Connect(
		function(input, gameProcessed)
			if Cmdbar:IsFocused() then
				if
					tabAllowed == true
					and input.KeyCode == Enum.KeyCode.Tab
					and topCommand ~= nil
				then
	...
	task.spawn(function()
		...
		then
			...
			if plr.Character and plr.Character:FindFirstChild('Head') then
				...
				teamChange = plr:GetPropertyChangedSignal('TeamColor')
					:Connect(function()
						if ESPenabled then
							espLoopFunc:Disconnect()
							addedFunc:Disconnect()
					...
					if COREGUI:FindFirstChild(plr.Name .. '_ESP') then
						...
						then
							local pos = math.floor(
								(
									getRoot(Players.LocalPlayer.Character).Position
									- getRoot(plr.Character).Position
								).magnitude
							)
							TextLabel.Text = 'Name: '
								.. plr.Name
...
function Locate(plr)
	task.spawn(function()
		...
		if
			plr.Character
			and plr.Name ~= Players.LocalPlayer.Name
			and not COREGUI:FindFirstChild(plr.Name .. '_LC')
		then
			local ESPholder = Instance.new('Folder')
...
Add_2.MouseButton1Click:Connect(function()
	if keySelected then
		if
			string.find(Cmdbar_2.Text, '\\\\')
			or string.find(Cmdbar_3.Text, '\\\\')
		then
			notify(
				'Keybind Error',
...
local function clicktpFunc()
	pcall(function()
		...
		local hipHeight = humanoid
			and humanoid.HipHeight > 0
			and (humanoid.HipHeight + 1)
	...
	for i, v in pairs(binds) do
		if v.COMMAND == 'clicktp' then
			...
			then
				clicktpFunc()
			elseif
				UserInputService:IsKeyDown(Enum.KeyCode[input:sub(14)])
				and Players.LocalPlayer.Character
			then
				clicktpFunc()
			end
...
function addPlugin(name)
	if
		name:lower() == 'plugin file name'
		or name:lower() == 'iy_fe.iy'
		or name == 'iy_fe'
	then
...
function LoadPlugin(val, startup)
	...
	function handlePluginError(plerror)
		notify(
			'Plugin Error',
			'An error occurred with the plugin, "'
				.. val
	...
	if plugin ~= nil then
		...
		addcmdtext('', val)
		addcmdtext(
			string.upper('--' .. plugin['PluginName']),
			val,
			plugin['PluginDescription']
		)
...
addcmd('jobid', {}, function(args, speaker)
	local jobId = 'Roblox.GameLauncher.joinGameInstance('
		.. PlaceId
		.. ', "'
		.. JobId
		.. '")'
...
addcmd('autorejoin', { 'autorj' }, function(args, speaker)
	local Dir = COREGUI:FindFirstChild('RobloxPromptGui')
		:FindFirstChild('promptOverlay')
	...
	function ToServer(User, Place)
		...
		then
			if retries < 3 then
				...
				notify(
					'Join Error',
					'Error while trying to join. Retrying ' .. retries .. '/3.'
				)
				ToServer(User, Place)
			else
...
addcmd('cframefly', { 'cfly' }, function(args, speaker)
	...
	CFloop = RunService.Heartbeat:Connect(function(deltaTime)
		local moveDirection = speaker.Character:FindFirstChildOfClass(
			'Humanoid'
		).MoveDirection * (CFspeed * deltaTime)
...
addcmd('swim', {}, function(args, speaker)
	if
		not swimming
		and speaker
		and speaker.Character
		and speaker.Character:FindFirstChildWhichIsA('Humanoid')
	then
...
addcmd(
	...
	function(args, speaker)
		...
		if speaker.Character then
			for i, _ in pairs(WayPoints) do
				if
					tostring(WayPoints[i].NAME):lower()
					== tostring(WPName):lower()
				then
					local x = WayPoints[i].COORD[1]
					local y = WayPoints[i].COORD[2]
					local z = WayPoints[i].COORD[3]
			...
			for i, _ in pairs(pWayPoints) do
				if
					tostring(pWayPoints[i].NAME):lower()
					== tostring(WPName):lower()
				then
...
addcmd('tweenwaypoint', { 'twp' }, function(args, speaker)
	...
	if speaker.Character then
		for i, _ in pairs(WayPoints) do
			...
			local z = WayPoints[i].COORD[3]
			if
				tostring(WayPoints[i].NAME):lower() == tostring(WPName):lower()
			then
				TweenService:Create(
		...
		for i, _ in pairs(pWayPoints) do
			if
				tostring(pWayPoints[i].NAME):lower() == tostring(WPName):lower()
			then
				TweenService:Create(
					getRoot(speaker.Character),
					TweenInfo.new(tweenSpeed, Enum.EasingStyle.Linear),
...
addcmd('showguis', {}, function(args, speaker)
	for i, v in
		pairs(speaker:FindFirstChildWhichIsA('PlayerGui'):GetDescendants())
	do
		if
			(v:IsA('Frame') or v:IsA('ImageLabel') or v:IsA('ScrollingFrame'))
			and not v.Visible
		then
			v.Visible = true
...
function deleteGuisAtPos()
	pcall(function()
		local guisAtPosition =
			Players.LocalPlayer.PlayerGui:GetGuiObjectsAtPosition(
				IYMouse.X,
				IYMouse.Y
			)
		for _, gui in pairs(guisAtPosition) do
	...
	oldhmmnc = hookmetamethod(game, '__namecall', function(self, ...)
		if
			self == TeleportService
				and getnamecallmethod():lower() == 'teleport'
...
addcmd('lastcommand', { 'lastcmd' }, function(args, speaker)
	if
		cmdHistory[1]:sub(1, 11) ~= 'lastcommand'
		and cmdHistory[1]:sub(1, 7) ~= 'lastcmd'
	then
...
addcmd('unpartesp', { 'nopartesp' }, function(args, speaker)
	if args[1] then
		...
		for i, v in pairs(workspace:GetDescendants()) do
			if
				v:IsA('BoxHandleAdornment')
				and v.Name == partEspName .. '_PESP'
			then
				v:Destroy()
			end
		end
...
addcmd('view', { 'spectate' }, function(args, speaker)
	...
	for i, v in pairs(players) do
		...
		viewChanged = workspace.CurrentCamera
			:GetPropertyChangedSignal('CameraSubject')
			:Connect(viewChangedFunc)
	end
end)
...
do
	...
	do
		function Keypress(action, state, input)
			keyboard[input.KeyCode.Name] = state == Enum.UserInputState.Begin
					and 1
				or 0
...
addcmd(
	...
	function(args, speaker)
		...
		notify(
			'Item(s) Deleted',
			'Deleted items with ClassName ' .. getstring(1)
		)
	end
)
...
addcmd('joindate', { 'jd' }, function(args, speaker)
	...
	notify('Loading', 'Hold on a sec')
	for i, v in pairs(players) do
		local user = game:HttpGet(
			'https://users.roblox.com/v1/users/' .. Players[v].UserId
		)
		local json = HttpService:JSONDecode(user)
		local date = json['created']:sub(1, 10)
		local splitDates = string.split(date, '-')
...
addcmd('clientbring', { 'cbring' }, function(args, speaker)
	...
	for i, v in pairs(players) do
		if Players[v].Character ~= nil then
			...
			wait()
			getRoot(Players[v].Character).CFrame = getRoot(speaker.Character).CFrame
				+ Vector3.new(3, 1, 0)
		end
	end
end)
...
addcmd('loopbring', {}, function(args, speaker)
	...
	for i, v in pairs(players) do
		task.spawn(function()
			if
				Players[v].Name ~= speaker.Name
				and not FindInTable(bringT, Players[v].Name)
			then
						...
						if Players:FindFirstChild(v) then
							...
							then
								getRoot(pchar).CFrame = getRoot(
									speaker.Character
								).CFrame + Vector3.new(distance, 1, 0)
							end
							wait(lDelay)
						else
		...
		if Players[v].Character ~= nil then
			...
			repeat
				...
				speaker.Character
					...
					:MoveTo(getRoot(Players[v].Character).Position)
			until Players[v].Character == nil
				or not getRoot(Players[v].Character)
				or walkto == false
		end
	end
end)
				...
				local success, response = pcall(function()
					...
					for waypointIndex, waypoint in pairs(waypoints) do
						...
						repeat
							distance = (
								waypointPosition
								- hum.Parent.PrimaryPart.Position
							).magnitude
							wait()
						until distance <= 5
					end
...
addcmd('muteboombox', {}, function(args, speaker)
	if
		not notifiedRespectFiltering and SoundService.RespectFilteringEnabled
	then
...
addcmd('god', {}, function(args, speaker)
	...
	speaker.Character, Cam.CameraSubject, Cam.CFrame =
		Char, nHuman, wait() and Pos
...
addcmd('invisible', { 'invis' }, function(args, speaker)
	...
	local invisDied
	invisDied = InvisibleCharacter:FindFirstChildOfClass('Humanoid').Died
		:Connect(function()
			Respawn()
	...
	function TurnVisible()
		...
		Player.Character.Animate.Disabled = true
		Player.Character.Animate.Disabled = false
		invisDied = Character:FindFirstChildOfClass('Humanoid').Died
			:Connect(function()
				Respawn()
				invisDied:Disconnect()
			end)
...
addcmd('nohead', { 'rhead', 'headless' }, function(args, speaker)
	if sethidden then
		...
		local rigType = tostring(char:FindFirstChildOfClass('Humanoid').RigType)
					== 'Enum.HumanoidRigType.R6'
				and 1
			or tostring(char:FindFirstChildOfClass('Humanoid').RigType)
					== 'Enum.HumanoidRigType.R15'
				and 2
...
addcmd('noarms', { 'rarms' }, function(args, speaker)
	if r15(speaker) then
		for i, v in pairs(speaker.Character:GetChildren()) do
			if
				v:IsA('BasePart') and v.Name == 'RightUpperArm'
				or v.Name == 'LeftUpperArm'
			then
				v:Destroy()
			end
		...
		for i, v in pairs(speaker.Character:GetChildren()) do
			if
				v:IsA('BasePart') and v.Name == 'Right Arm'
				or v.Name == 'Left Arm'
			then
				v:Destroy()
			end
...
addcmd('lay', { 'laydown' }, function(args, speaker)
	local Human = speaker.Character
		and speaker.Character:FindFirstChildOfClass('Humanoid')
	if not Human then
		return
	end
	Human.Sit = true
...
addcmd('nosit', {}, function(args, speaker)
	...
	noSit = Players.LocalPlayer.Character
		:FindFirstChildOfClass('Humanoid')
		:GetPropertyChangedSignal('Sit')
		:Connect(noSitFunc)
...
addcmd('autojump', { 'ajump' }, function(args, speaker)
	...
	local function autoJump()
		if Char and Human then
			local check1 = workspace:FindPartOnRay(
				Ray.new(
					Human.RootPart.Position - Vector3.new(0, 1.5, 0),
	...
	HumanModCons.ajLoop = (
		HumanModCons.ajLoop
		and HumanModCons.ajLoop:Disconnect()
		and false
	) or RunService.RenderStepped:Connect(autoJump)
	HumanModCons.ajCA = (
		HumanModCons.ajCA
		and HumanModCons.ajCA:Disconnect()
		and false
	)
...
addcmd('edgejump', { 'ejump' }, function(args, speaker)
	...
	HumanModCons.ejLoop = (
		HumanModCons.ejLoop
		and HumanModCons.ejLoop:Disconnect()
		and false
	) or RunService.RenderStepped:Connect(edgejump)
	HumanModCons.ejCA = (
		HumanModCons.ejCA
		and HumanModCons.ejCA:Disconnect()
		and false
	)
		or speaker.CharacterAdded:Connect(function(nChar)
			Char, Human = nChar, nChar:WaitForChild('Humanoid')
...
addcmd(
	...
	function(args, speaker)
		...
		if not Human or not Animate then
			return notify(
				'Refresh Animations',
				'Failed to get Animate/Humanoid'
			)
		end
...
addcmd(
	...
	function(args, speaker)
		...
		for i, v in pairs(players) do
			local char = Players[v].Character
			local pos = char
				and (getRoot(char) or char:FindFirstChildWhichIsA('BasePart'))
			pos = pos and pos.Position
			if not pos then
				return notify('Getposition Error', 'Missing character')
			end
			local roundedPos = math.round(pos.X)
				.. ', '
				.. math.round(pos.Y)
				.. ', '
				.. math.round(pos.Z)
	...
	if args[1] and isNumber(args[1]) then
		if hookmetamethod then
			...
			index = hookmetamethod(game, '__index', function(self, key)
				...
				if
					(keyclean == 'WalkSpeed' or keyclean == 'walkSpeed')
...
addcmd('loopspeed', { 'loopws' }, function(args, speaker)
	...
	if isNumber(speed) then
		...
		HumanModCons.wsLoop = (
			HumanModCons.wsLoop
			and HumanModCons.wsLoop:Disconnect()
			and false
		)
			or Human:GetPropertyChangedSignal('WalkSpeed')
				:Connect(WalkSpeedChange)
		HumanModCons.wsCA = (
			HumanModCons.wsCA
			and HumanModCons.wsCA:Disconnect()
			and false
		)
			or speaker.CharacterAdded:Connect(function(nChar)
				Char, Human = nChar, nChar:WaitForChild('Humanoid')
	...
	if args[1] and isNumber(args[1]) then
		if hookmetamethod then
			...
			index = hookmetamethod(game, '__index', function(self, key)
				local keyclean = key:gsub('\0', '')
				if
					(keyclean == 'JumpPower' or keyclean == 'jumpPower')
					and self:IsA('Humanoid')
					and self:IsDescendantOf(char)
					and not checkcaller()
				then
...
addcmd('loopjumppower', { 'loopjp', 'loopjpower' }, function(args, speaker)
	...
	if isNumber(jpower) then
		...
		local function JumpPowerChange()
			if Char and Human then
				if
					speaker.Character:FindFirstChildOfClass('Humanoid').UseJumpPower
				then
					speaker.Character:FindFirstChildOfClass('Humanoid').JumpPower =
						jpower
				else
		...
		HumanModCons.jpLoop = (
			HumanModCons.jpLoop
			and HumanModCons.jpLoop:Disconnect()
			and false
		)
			or Human:GetPropertyChangedSignal('JumpPower')
				:Connect(JumpPowerChange)
		HumanModCons.jpCA = (
			HumanModCons.jpCA
			and HumanModCons.jpCA:Disconnect()
			and false
		)
			or speaker.CharacterAdded:Connect(function(nChar)
				Char, Human = nChar, nChar:WaitForChild('Humanoid')
...
addcmd(
	...
	function(args, speaker)
		for i, v in
			pairs(speaker:FindFirstChildOfClass('Backpack'):GetDescendants())
		do
...
addcmd('console', {}, function(args, speaker)
	...
	local _, str = pcall(function()
		return game:HttpGet(
			'https://raw.githubusercontent.com/DarkNetworks/Infinite-Yield/main/console.lua',
			true
		)
	end)
...
addcmd('explorer', { 'dex' }, function(args, speaker)
	...
	loadstring(
		game:HttpGet(
			'https://raw.githubusercontent.com/DarkNetworks/Infinite-Yield/main/dex.lua'
		)
	)()
end)
...
addcmd('remotespy', { 'rspy' }, function(args, speaker)
	...
	loadstring(
		game:HttpGet(
			'https://raw.githubusercontent.com/DarkNetworks/Infinite-Yield/main/SimpleSpyV3/main.lua'
		)
	)()
end)
...
addcmd('audiologger', { 'alogger' }, function(args, speaker)
	...
	loadstring(
		game:HttpGet(
			'https://raw.githubusercontent.com/DarkNetworks/Infinite-Yield/main/audiologger.lua',
			true
		)
	)()
end)
...
addcmd('loopgoto', {}, function(args, speaker)
	...
	for i, v in pairs(players) do
		...
		repeat
			if Players:FindFirstChild(v) then
				if Players[v].Character ~= nil then
					getRoot(speaker.Character).CFrame = getRoot(
						Players[v].Character
					).CFrame + Vector3.new(distance, 1, 0)
				end
				wait(lDelay)
	...
	for i, v in pairs(players) do
		...
		headSit = RunService.Heartbeat:Connect(function()
			if
				...
				and getRoot(speaker.Character)
				and speaker.Character:FindFirstChildOfClass('Humanoid').Sit
					== true
			then
				getRoot(speaker.Character).CFrame = getRoot(
					Players[v].Character
				).CFrame * CFrame.Angles(0, math.rad(0), 0) * CFrame.new(
					0,
					1.6,
					0.4
				)
...
addcmd('blockhats', {}, function(args, speaker)
	for _, v in
		pairs(
			speaker.Character:FindFirstChildOfClass('Humanoid'):GetAccessories()
		)
	do
	...
	for _, v in pairs(players) do
		...
		bangLoop = RunService.Stepped:Connect(function()
			pcall(function()
				...
				getRoot(Players.LocalPlayer.Character).CFrame = otherRoot.CFrame
					* bangOffet
			end)
		end)
	end
...
addcmd('bringpartclass', { 'bpc' }, function(args, speaker)
	for i, v in pairs(workspace:GetDescendants()) do
		if
			v.ClassName:lower() == getstring(1):lower() and v:IsA('BasePart')
		then
			v.CFrame = getRoot(speaker.Character).CFrame
		end
	end
...
addcmd('instantproximityprompts', { 'instantpp' }, function(args, speaker)
	if fireproximityprompt then
		...
		wait(0.1)
		PromptButtonHoldBegan = ProximityPromptService.PromptButtonHoldBegan:Connect(
			function(prompt)
	...
	if args[1] and speaker:FindFirstChildOfClass('Backpack') then
		...
		local RST = RunService.RenderStepped:Connect(function()
			if speaker:FindFirstChildOfClass('Backpack') then
				for i, v in
					pairs(
						speaker:FindFirstChildOfClass('Backpack'):GetChildren()
					)
				do
...
addcmd('copytools', {}, function(args, speaker)
	...
	for i, v in pairs(players) do
		task.spawn(function()
			for i, v in
				pairs(
					Players[v]:FindFirstChildOfClass('Backpack'):GetChildren()
				)
			do
				if v:IsA('Tool') or v:IsA('HopperBin') then
...
addcmd('clearhats', { 'cleanhats' }, function(args, speaker)
	if firetouchinterest then
		...
		for _, accessory in
			ipairs(Character:FindFirstChildOfClass('Humanoid'):GetAccessories())
		do
			accessory:Destroy()
		end
...
addcmd('vr', {}, function(args, speaker)
	...
	loadstring(
		game:HttpGet(
			'https://raw.githubusercontent.com/DarkNetworks/Infinite-Yield/main/vr.lua'
		)
	)()
end)
...
addcmd('replaceroot', { 'replacerootpart' }, function(args, speaker)
	if
		...
		and speaker.Character:FindFirstChild('HumanoidRootPart')
	then
		local Char = speaker.Character
...
addcmd('dupetools', { 'clonetools' }, function(args, speaker)
	...
	for i = 1, LOOP_NUM do
		...
		Human.RootPart.Anchored = speaker:ClearCharacterAppearance(wait(0.1))
			or true
		...
		if i == LOOP_NUM or i % 5 == 0 then
			...
			if type(firetouchinterest) == 'function' then
				for _, v in ipairs(Tools) do
					v.Anchored = not firetouchinterest(
						v,
						HRP,
						1,
...
addcmd('givetool', { 'givetools' }, function(args, speaker)
	...
	speaker:ClearCharacterAppearance()
	task.spawn(function()
		speaker.CharacterAdded:Wait():WaitForChild('Humanoid').RootPart.CFrame = wait()
			and hrppos
	end)
...
addcmd(
	...
	function(args, speaker)
		...
		local root = getRoot(speaker.Character)
			or speaker.Character:FindFirstChildWhichIsA('BasePart')
		...
		if args[1] then
			...
			for _, descendant in ipairs(workspace:GetDescendants()) do
				if
					descendant:IsA('TouchTransmitter')
					and descendant.Name == name
				then
					touch(descendant)
				end
...
addcmd('droppabletools', {}, function(args, speaker)
	...
	if speaker:FindFirstChildOfClass('Backpack') then
		for _, obj in
			pairs(speaker:FindFirstChildOfClass('Backpack'):GetChildren())
		do
			if obj:IsA('Tool') then
				obj.CanBeDropped = true
			end
...
function attach(speaker, target)
	if tools(speaker) then
		...
		local tool = speaker
			:FindFirstChildOfClass('Backpack')
			:FindFirstChildOfClass('Tool') or speaker.Character:FindFirstChildOfClass(
			'Tool'
		)
		tool.Parent = char
	...
	if tools(speaker) then
		if target ~= nil then
			...
			until not getRoot(target.Character)
				or not getRoot(speaker.Character)
			speaker.CharacterAdded
				:Wait()
	...
	for _, v in ipairs(getPlayer(args[1], speaker)) do
		...
		task.spawn(function()
			while
				...
				and speaker.Character
				and v.Character
				and Tool.Parent == speaker.Character
			do
				local Human =
					...
					v1 = (
						(
							...
							and firetouchinterest(
								...
								1,
								(RS.Wait(RS) and nil)
									or firetouchinterest(Handle, v1, 0)
							)
							and nil
						) or v1
					) or v1
...
addcmd('tpwalk', { 'teleportwalk' }, function(args, speaker)
	...
	while tpwalking and chr and hum and hum.Parent do
		...
		if hum.MoveDirection.Magnitude > 0 then
			if args[1] and isNumber(args[1]) then
				chr:TranslateBy(
					hum.MoveDirection * tonumber(args[1]) * delta * 10
				)
			else
...
addcmd('tp', { 'teleport' }, function(args, speaker)
	...
	for i, v in pairs(players1) do
		if
			...
			and getRoot(Players[players2[1]].Character)
		then
			if
				speaker.Character:FindFirstChildOfClass('Humanoid')
				and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart
...
function x(v)
	if v then
		for _, i in pairs(workspace:GetDescendants()) do
			if
				i:IsA('BasePart')
				and not i.Parent:FindFirstChildOfClass('Humanoid')
				and not i.Parent.Parent:FindFirstChildOfClass('Humanoid')
			then
...
addcmd('walltp', {}, function(args, speaker)
	...
	local function touchedFunc(hit)
		...
		if
			hit:IsA('BasePart')
			and hit.Position.Y
				> Root.Position.Y - speaker.Character:FindFirstChildOfClass(
					'Humanoid'
				).HipHeight
		then
				...
				if not gameProcessedEvent then
					if
						(
							...
							and UserInputService:IsKeyDown(Enum.KeyCode.Equals)
						)
						or (
...
addcmd('headsize', {}, function(args, speaker)
	...
	for i, v in pairs(players) do
		if
			Players[v] ~= speaker
			and Players[v].Character:FindFirstChild('Head')
		then
...
addcmd('rolewatchleave', { 'unrolewatch' }, function(args, speaker)
	...
	notify(
		'Rolewatch',
		RolewatchData.Leave and 'Leave has been Enabled'
			or 'Leave has been Disabled'
	)
end)
...
addcmd('freezeunanchored', { 'freezeua' }, function(args, speaker)
	if sethidden then
		...
		local function FREEZENOOB(v)
			if
				v:IsA('BasePart' or 'UnionOperation')
			...
			then
				...
				if
					speaker.Character and v:IsDescendantOf(speaker.Character)
				then
					BADD = true
				end
				if BADD == false then
...
task.spawn(function()
	...
	if success then
		...
		if
			latestVersionInfo.Announcement
			and latestVersionInfo.Announcement ~= ''
		then
			...
			wait(1)
			AnnGUI:TweenPosition(
				UDim2.new(0.5, -180, 0, 150),
				'InOut',
				'Quart',
				0.5,
				true,
...
task.spawn(function()
	...
	TweenService
		:Create(IntroBackground, OutInfo, { BackgroundTransparency = 1 })
		:Play()
